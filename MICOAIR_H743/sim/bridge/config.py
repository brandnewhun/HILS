# -*- coding: utf-8 -*-
"""
HILS 브릿지 설정(Micoair H743 V2 판) — 이 파일 하나만 고치면 대부분의 운용 파라미터가 바뀌도록 모아둔 곳.
(ICD-PXTR-HILS-001 / EICD-PXTR-HILS-001 참조 — 문서 원본은 ../../../PX6C/ 에 있음)

PX6C 판(../../../PX6C/sim/bridge/config.py)과의 차이는 "FCC 보드 식별" 절과 시리얼 포트
주석뿐이다. HIL_SENSOR 주입·MANUAL_CONTROL·NSH·장치 ID 대조는 PX4 내부 규약이라 보드와 무관.
...
# Windows 장치관리자(포트, COM & LPT)에서 Micoair H743 V2의 USB-C(CDC-ACM)가 잡는 COM 번호로
# 바꿔서 사용. (PX4 보드 설정 micoair_h743-v2에 CDCACM_AUTOSTART가 포함되어 있어 PX6c와
# 같이 USB 직결로 MAVLink가 바로 올라온다.)
SERIAL_PORT = "COM11"

# USB-C 포트(가상 CDC-ACM)는 USB native 속도라 baudrate가 무시된다.
# TELEM UART를 USB-시리얼(FTDI 등) 변환기로 뺄 경우에만 EICD-01 3.1.1절 값(921600)을 쓴다.
# Micoair H743 V2의 PX4 UART 배정(boards/micoair/h743-v2/default.px4board 기준):
#   TEL1=/dev/ttyS0(USART1)  TEL2=/dev/ttyS3  TEL3=/dev/ttyS4  TEL4=/dev/ttyS7
#   GPS1=/dev/ttyS2  GPS2=/dev/ttyS1  RC=/dev/ttyS5  URT6=/dev/ttyS6
# → TELEM으로 연결하려면 OFP의 MAV_{0,1,2}_CONFIG가 그 포트를 가리키는지 먼저 확인.
SERIAL_BAUD = 115200  # USB 포트 연결 시 pymavlink 요구사항상 값 자체는 있어야 하나 무시됨

# ── FCC 보드 식별 — 이 폴더(MICOAIR_H743)는 Micoair H743 V2 전용 ─────────────────────
# main.py의 identify_fcc()가 시작 시 HEARTBEAT.autopilot / AUTOPILOT_VERSION / SYS_HITL 을
# 읽어 session.json에 남기고, 아래 기대값과 다르면 경고(또는 중단)한다. 보드 교체기의
# 흔한 실수 세 가지를 시작 5초 안에 잡기 위함:
#   1) 출고 펌웨어(ArduPilot)가 그대로 있음 — ArduPilot에는 HIL_SENSOR 경로가 없어 브릿지 불가
#   2) SYS_HITL이 0 — Micoair H743 V2는 파라미터를 SD카드(/fs/microsd/params)에 저장하므로
#      SD가 없으면 재부팅마다 초기화된다(브릿지는 시작/RESET마다 FC를 재부팅함 → SD 필수)
#   3) 다른 보드/펌웨어 버전 — AUTOPILOT_VERSION의 보드 ID·git 해시로 세션 간 재현성 확보
FCC_BOARD = "micoair_h743-v2"      # PX4 보드 타깃명(PX4 1.16.0+, 1.15.x에는 보드 폴더 이식 필요)
FCC_BOARD_LABEL = "Micoair H743 V2"
EXPECTED_AUTOPILOT = 12            # MAV_AUTOPILOT_PX4 (ArduPilot=3, 제네릭=0)
ABORT_ON_WRONG_AUTOPILOT = True    # PX4가 아니면 즉시 종료(False면 경고만 하고 진행)
EXPECTED_SYS_HITL = 1              # None이면 검사 생략
...
# ── FC(Micoair H743 V2) 자동 재부팅 — "시뮬레이션 리셋 = FC 리셋" 규칙 ──────────
# ★ 이 보드는 파라미터가 SD카드에 있다. SD 없이 재부팅하면 SYS_HITL/SYS_AUTOSTART 등이
#   전부 기본값으로 돌아가 HIL이 풀린다(실물 센서로 부팅). identify_fcc()가 잡아준다.
