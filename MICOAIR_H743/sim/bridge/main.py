...
연결만 한다(이 파일 자체에는 로직을 넣지 않는다. 로직을 바꾸려면 해당 모듈만 고칠 것):
  ...
  MavlinkLink(mavlink_link.py)   — FCC(Micoair H743 V2)와의 EICD-01 물리/논리 인터페이스
...
def reboot_fcc_and_reconnect(link, settle_s=2.0, heartbeat_wait_s=30.0):
    """FC를 NSH `reboot`으로 재부팅하고, USB 시리얼이 다시 올라와 첫 HEARTBEAT가
...
def identify_fcc(link, session, wait_s=5.0):
    """보드 교체용 시작 진단 — 연결된 FC가 "PX4 OFP가 올라간 Micoair H743 V2, HIL 모드"인지
    확인하고 session.json에 남긴다(config의 "FCC 보드 식별" 절 참조).
      · HEARTBEAT.autopilot ≠ PX4  → 출고 ArduPilot 등. HIL_SENSOR 경로가 없으므로 중단(설정 가능)
      · AUTOPILOT_VERSION          → 펌웨어 버전/보드 ID/git 해시(세션 간 비교용, 실패해도 진행)
      · SYS_HITL ≠ 1               → SD카드 없음/파라미터 초기화 의심. 경고만.
    """
    c = config
    print("[main] FC 식별 중(기대: %s / PX4 / SYS_HITL=%s)..." % (c.FCC_BOARD_LABEL, c.EXPECTED_SYS_HITL))
    try:
        link.request_autopilot_version()
    except Exception as e:
        print("[main] AUTOPILOT_VERSION 요청 실패(%s) — 계속" % type(e).__name__)
    deadline = time.time() + wait_s
    while time.time() < deadline:
        link.poll_incoming()
        if "autopilot" in link.fcc_identity and "flight_sw_version" in link.fcc_identity:
            break
        time.sleep(0.05)
    ident = dict(link.fcc_identity)
    if c.EXPECTED_SYS_HITL is not None:
        v = link.read_param("SYS_HITL")
        ident["SYS_HITL"] = int(v) if v is not None else None
    v = link.read_param("SYS_AUTOSTART")
    ident["SYS_AUTOSTART"] = int(v) if v is not None else None
    ident["expected_board"] = c.FCC_BOARD
    session.update_session(fcc_identity=ident)

    ap = ident.get("autopilot")
    ap_name = {12: "PX4", 3: "ArduPilot", 0: "GENERIC", 8: "INVALID"}.get(ap, "unknown(%s)" % ap)
    print("    autopilot        = %s" % ap_name)
    print("    firmware         = %s (board_version=%s, git=%s)" % (
        ident.get("flight_sw_version", "?"), ident.get("board_version", "?"),
        ident.get("flight_custom_version", "?")))
    print("    SYS_HITL         = %s   SYS_AUTOSTART = %s" % (ident.get("SYS_HITL"), ident.get("SYS_AUTOSTART")))

    if ap is not None and ap != c.EXPECTED_AUTOPILOT:
        msg = ("FC 오토파일럿이 %s 입니다(기대: PX4). Micoair 보드 출고 펌웨어(ArduPilot)에는 HIL_SENSOR "
               "주입 경로가 없어 이 브릿지를 쓸 수 없습니다 — PX4 부트로더(DFU)와 OFP(%s 타깃)를 먼저 올려주세요."
               % (ap_name, c.FCC_BOARD))
        print("[main] ★ " + msg)
        session.event("fcc_identity", "wrong autopilot: %s" % ap_name)
        if getattr(c, "ABORT_ON_WRONG_AUTOPILOT", True):
            raise SystemExit(2)
    if c.EXPECTED_SYS_HITL is not None and ident.get("SYS_HITL") != c.EXPECTED_SYS_HITL:
        print("[main] ★ SYS_HITL=%s (기대 %s). 이 보드는 파라미터를 SD카드에 저장합니다 — SD카드가 없거나 "
              "포맷이 안 됐으면 재부팅마다 초기화됩니다. SD 장착 후 `param set SYS_HITL 1; param save` 하세요."
              % (ident.get("SYS_HITL"), c.EXPECTED_SYS_HITL))
        session.event("fcc_identity", "SYS_HITL mismatch: %s" % ident.get("SYS_HITL"))
    return ident
...
def main():
    ...
    print("[main] FCC(%s) 연결 시도: %s (baud=%s)" % (config.FCC_BOARD_LABEL, config.SERIAL_PORT, config.SERIAL_BAUD))
    ...
    # 진단 0 — 연결된 FC가 기대한 보드/펌웨어/HIL 모드인지(보드 교체 진단, config 참조)
    identify_fcc(link, session)
